    ...
    async fn process(&mut self, msg: Message) -> Result<()> {
        match msg {
            ...
            Message::Database(msg) => {
                // The database actor should only receive messages when the application is running
                // as the control node when the db handle should be `Some`.
                self.db
                    .as_mut()
                    .expect("database handle should not be None")
                    .send(msg)
                    .await
            },
            Message::Drive(msg) => {
