...
use super::{DiscState, OpticalDrive, OpticalDriveState, OpticalDriveStatus};
...
impl DriveActor {
    ...
    fn new(serial_number: String, rx: mpsc::Receiver<DriveActorMessage>) -> Self {
        Self { serial_number, state: OpticalDriveState::Idle, rx }
    }
    ...
    fn get_state(&self, tx: oneshot::Sender<OpticalDriveStatus>) -> Result<()> {
        ...
        tx.send(status)
            .map_err(|_| Error::DriveActorChannel { error: ChannelError::OneShotSend })?;
