...
/// [`State::Copying`] state.
fn copying_header<'a>(component: &'a DriveComponent, service: &'a CopyService) -> Element<'a> {
...
/// Generates the footer UI element for the drive component.
fn footer_component(service: &CopyService) -> Element<'_> {
...
fn idle_content<'a>(component: &'a DriveComponent, service: &'a CopyService) -> Element<'a> {
...
/// This will panic if the service's drive is `None`. This function should only ever be called for 
/// a service with a valid drive.
fn idle_header<'a>(component: &'a DriveComponent, service: &'a CopyService) -> Element<'a> {
    // Should be safe to expect a value here.
    let drive = service.drive().expect("expected a valid drive");
...
fn reset_header<'a>(component: &'a DriveComponent, service: &'a CopyService) -> Element<'a> {
