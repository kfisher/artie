...
impl Settings {
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        let contents = fs::read_to_string(path)
            .map_err(|error| Error::FileIoError { path: path.to_owned(), error })?;
        let settings: Settings = toml::from_str(&contents)
            .map_err(|error| Error::SerializationError { 
    ...
    pub fn update_copy_services(&mut self, services: &Vec<CopyService>) {
        self.copy_services = services.iter()
            .map(|service| CopyServiceSettings::new(
                service.name.clone(),
                service.drive.serial_number.clone()
            ))
            .collect();
    }
}
